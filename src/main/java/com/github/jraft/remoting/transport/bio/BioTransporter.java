...
public class BioTransporter implements Transporter {
    @Override
    public Server bind(String host, int port)  throws RemotingException {
        return new BioServer(host,port);
    }

    @Override
    public Client connect(String host, int port)  throws RemotingException {
        return new BioClient(host, port);
    }
}
